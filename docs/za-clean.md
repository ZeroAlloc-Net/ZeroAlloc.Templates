...
| [ZA.Validation](https://validation.zeroalloc.net) | `MyApp.Application/CreateOrder/CreateOrderCommand.cs`, `OrderItem.cs` | `[Validate]` attributes on the command + nested item record. ZA.Validation's source generator emits `CreateOrderCommandValidator` and `OrderItemValidator` at build time. The thin facade at [`CreateOrderValidator.cs`](../content/za-clean/src/MyApp.Application/CreateOrder/CreateOrderValidator.cs) delegates to the generated validator and maps the first failure to `ApplicationError("validation.failed", ...)` — keeping `CreateOrderHandler.Handle` unchanged. The "at least one item" rule is enforced in the facade because `[NotEmpty]` is currently string-only in the upstream generator. |
