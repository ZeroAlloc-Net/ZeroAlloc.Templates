...
| [ZA.Validation](https://validation.zeroalloc.net) | `MyApp.Application/CreateOrder/CreateOrderCommand.cs` | `[Validate]` + per-property attrs (`[Required]`, `[Range]`). **Currently decorative** — the generator does not ship in the published nupkg yet; validation runs no-op until the upstream fix lands. Handlers call into validation explicitly in the interim. |
